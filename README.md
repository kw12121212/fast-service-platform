...
这个项目不是普通业务系统，也不是通用低代码平台。它的目标是在 `Lealone-Platform` 的基础上沉淀一套对 AI 友好的企业应用组件，让 AI 在明确约束下生成可运行的企业内部管理系统。

## 项目定位

- 直接使用者是 `AI`
- 人类通过 `自然语言` 描述需求，可补充 `原型图片` 和 `UI 参考网站`
- V1 只面向 `企业内部管理应用`
- V1 只生成 `单体应用`
...
- 已完成 backend 和 frontend 的两次 spec-driven change 并归档
- 当前没有活动中的 spec-driven change
- 下一步更适合继续抽象 AI 可复用的前端资源模块层，或扩展后端企业组件
